...
class TicTacToeEnv(gym.Env):
    ...
    abi = json.load('./../contracts/TicTacToe.json')
    ...
    def reset(self, address=None, game_id=None):
        if not address and not game_id:
            ...
            # load from contract address and game ID
            contract_instance = w3.eth.contract(address=address, abi=self.abi)
            self.players = [Player('1', Token('X', 1)), Player('2', Token('O', -1))
            self.current_player_num = 1 # autoplayer is always 2nd
            game_info = contract_instance.functions.games(game_id).call()
            board_from_game = game_info.board
            board = []
            num_turns = 0
            for row in board_from_game:
                for square in row:
                    if square == 0:
                        board.append([Token('.', 0)])
                    elif square == 1:
                        board.append([Token('X', 1)])
                        num_turns += 1
                    else:
                        board.append([Token('O', -1)])
                        num_turns +=1
            self.board = board
            self.turns_taken = num_turns
